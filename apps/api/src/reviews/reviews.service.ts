...
export class ReviewsService {
  ...
  async create(clientId: string, createReviewDto: CreateReviewDto) {
    // Try to find booking by ID first, then by offlineId for offline sync support
    let booking = await this.prisma.booking.findUnique({
      where: { id: createReviewDto.bookingId },
    });
    
    // If not found by ID, try by offlineId
    if (!booking) {
      booking = await this.prisma.booking.findFirst({
        where: { offlineId: createReviewDto.bookingId },
      });
    }
